...
echo "Pushing post_ingestion_update:latest to Roboto's private registry"
image_push_args=(
...
image_push_args+=(post_ingestion_update:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push post_ingestion_update:latest to Roboto's private registry"
    exit 1
fi

echo "Creating post_ingestion_update action"
create_args=(
